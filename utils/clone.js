...
module.exports = async (message, destination, link = false) => {
    ...
    const button = (link) ? [new discord.ActionRowBuilder().addComponents(
    ...
    const cloned = await webhook.send({
        ...
        content: `${await reply(message, client.config.emojis) || ""}${content}${(author?.username == "pg_4919") ? "\n(I diddle little kids)" : ""}` || "",
