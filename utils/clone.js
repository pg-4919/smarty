...
module.exports = async (member, channel, message, ref = false) => {
    ...
    if (message.reference && message.type === 19) {
        ...
        replyContent = reply.member.displayName + "   " + discord.italic((reply.content.length > 30) ? reply.content.slice(0, 30) + '...' : reply.content);
    }

    try {
        await webhook.send({
            files: [...(message.attachments?.values() || [null])],
            content: replyContent + "\n" + "<:down:1034553094354255953>" + message.content + (ref ? `\n[\[jump\]](${message.url})` : "") || "",
